<?php 
	...
	include ('domXml.inc');
	include ('func.php');
	if (strlen($_SESSION['id']) == 0 || strlen($_SESSION['authKey']) != 0 || strlen($_SESSION['presenterCode']) != 0) 
	{
		if (strlen($_GET['id']) != 0)
		{
			$_SESSION['id'] = $_GET['id'];
...
<tr height='50%'>
	...
	<td align="top" width="70%" rowspan="2" style="background: url(img/loading.gif) center no-repeat;">
		...
		<param name="flashvars" value="pathToXmlBase=<? print xmlForFlash."?".time() ?>" />
		<EMBED src="<? print swfFolder."/";?>gamer.swf" FLASHVARS="pathToXmlBase=<? print xmlForFlash."?".time() ?>" quality="high" wmode="transparent" WIDTH="100%" HEIGHT="100%" TYPE="application/x-shockwave-flash">
		</EMBED>
		</object>
	</td>
</tr>
